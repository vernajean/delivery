...
if (isset($_POST['ready'])) {
  ...
  if ($result) {
    ...
    echo "<script>showToast('Parcel successfully handed to the courier.'); window.location = 'orders.php';</script>";
  } else {
    echo "<script>showToast('There\'s an error on updating data.'); window.location = 'orders.php';</script>";
  }
}
...
if (isset($_POST['decline'])) {
  ...
  if ($result) {
    ...
    $logSql = "INSERT INTO logs (account_id, activity) VALUES ('$id','$activity')";
    mysqli_query($connection, $logSql);
    echo "<script>showToast('Item has been declined!'); window.location = 'orders.php';</script>";
  } else {
    echo "<script>showToast('There\'s an error on declining item.'); window.location = 'orders.php';</script>";
  }
}
?>
            ...
            <div class="card-body">
              <table id="example1" class="table table-bordered table-striped">
                ...
                <tbody>
                  ...
                  <?php
                  $query = "SELECT o.*, o.id AS order_id, a.*, p.image, p.description, p.product_name, p.price, p.quantity pqty, c.fullname AS courier_name
                              FROM orders AS o
                              INNER JOIN product AS p ON o.product_id = p.id
                              INNER JOIN accounts AS a ON a.id = o.buyer_id
                              LEFT JOIN accounts AS c ON o.courier_id = c.id
                  ...
                  if ($result->num_rows > 0) {
                    while ($row = $result->fetch_assoc()) {
                      ...
                      <tr>
                        ...
                        <td>
                          <?php if ($row['order_status'] == 1) { ?>
                            <button type="submit" class="btn btn-primary" data-toggle="modal" data-target="#modal-ready">Ready
                              to Deliver</button>
                            ...
                            <button type="button" class="btn btn-danger" data-toggle="modal"
                              data-target="#modal-decline">Decline</button>
